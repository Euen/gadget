...
-spec register(gadget_repo_tools:name(),
               gadget_repo_tools:tool(),
               gadget_repo_tools:token()) ->
  gadget_repo_tools:repo_tool().
...
-spec unregister(gadget_repo_tools:name(),
                 gadget_repo_tools:tool()) ->
  non_neg_integer().
...
-spec all() ->
  [gadget_repo_tools:repo_tool()].
all() ->
  sumo:find_all(gadget_repo_tools).

%% @doc returns a particular repo-tool, if found
-spec fetch(gadget_repo_tools:name(),
            gadget_repo_tools:tool()) ->
  notfound | gadget_repo_tools:repo_tool().
fetch(Name, Tool) ->
